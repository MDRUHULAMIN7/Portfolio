...
export default function DeleteConfirmModal({ isOpen, project, onClose, onDelete }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/experience/${project.id}/delete`, {
        method: "DELETE",
  ...
  return (
    <ModalWrapper isOpen={isOpen} onClose={onClose}>
      ...
      <p className="text-gray-300 mb-2">Are you sure you want to delete this experience?</p>

      <div className="bg-gray-800 p-3 rounded-lg mb-6">
        <p className="text-cyan-400 font-medium">{project?.designation}</p>
        <p className="text-gray-400 text-sm mt-1">{project?.company}</p>
      </div>
