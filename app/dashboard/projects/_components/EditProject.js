...
export const EditProject = ({ projectData, projectId }) => {
  ...
  useEffect(() => {
    if (projectData) {
      setHasChanges(
        JSON.stringify(defaultValues) !== JSON.stringify(watchAllFields)
      );
    }
  ...
  return (
    <form
      ...
      className="max-w-6xl bg-[#1f2937] rounded-lg shadow-md mx-auto p-6 space-y-8"
    >
      <Heading
        title1={"Update Project"}
        title2={"Edit & Update Project"}
      ></Heading>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div>
          ...
          <input
            {...register("meta.type", { required: "Type is required" })}
            className="input-field"
        ...
        <div>
          ...
          <input
            {...register("meta.service", { required: "Service is required" })}
            className="input-field"
        ...
        <div>
          ...
          <input
            {...register("meta.endDate", { required: "End date is required" })}
            type="date"
            className="input-field"
          />
          {errors.meta?.endDate && (
      ...
      <div>
        ...
        {linkFields.map((field, idx) => (
          <div
            key={field.id}
            className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-2"
          >
            <input
              {...register(`links.${idx}.live`)}
              className="input-field"
              placeholder="Live URL"
            />
            <input
              {...register(`links.${idx}.repo`)}
              className="input-field"
      ...
      <div>
        ...
        {techFields.map((field, idx) => (
          ...
          >
            <input
              {...register(`techStack.${idx}.name`)}
              placeholder="Tech Name"
              className="input-field"
            />
            <input
              {...register(`techStack.${idx}.value`)}
              placeholder="Value"
              className="input-field"
            />
          </div>
      ...
      <div>
        ...
        {featureGroups.map((group, gIdx) => (
          <div
            key={group.id}
            className="border border-gray-700 p-4 rounded-lg mb-2"
          >
            <input
              {...register(`features.${gIdx}.group`)}
        ...
        <button
          type="button"
          onClick={() => addFeatureGroup({ group: "", items: [{ text: "" }] })}
          className="btn-add"
        >
